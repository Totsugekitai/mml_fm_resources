...
//------------------------------------------------------------------------
CMouseEventResult CDispSlider::onMouseDown (CPoint &where, const long &button)
{
	cnt=0;
	oldVal	= value;

#if DEBUG
	DebugPrint("exec CDispSlider::onMouseDown");
#endif
	if (!bMouseEnabled)
		return kMouseEventNotImplemented;

//	if(button==-1) button = pParentFrame->getCurrentMouseButtons ();
...
//------------------------------------------------------------------------
CMouseEventResult CDispSlider::onMouseUp (CPoint& where, const long& buttons)
{
	endEdit ();	
...
//------------------------------------------------------------------------
CMouseEventResult CDispSlider::onMouseMoved (CPoint& where, const long& buttons)
{
	int ivalue;
	
	if ( (buttons & kLButton) != 0 && (buttons & kAlt) == 0 )
	{
		oldVal=value;
...
//------------------------------------------------------------------------
bool CDispSlider::onWheel (const CPoint &where, const float &distance, const long &buttons)
{
	#if DEBUG
	...
	//value += distance / 0.1f * getWheelInc();
	setDirty(true);
	bounceValue();
	#if DEBUG
